...
// Stream is a wrapper used to implement the MsgReadWriter
...
func newStream(useDL bool, lp2pStream inet.Stream) p2p.MsgReadWriter {
	return &Stream{
		UseDeadline: useDL,
...
func (stream *Stream) ReadMsg() (p2p.Msg, error) {
...
// to lilbp2p streams.
func (stream *Stream) WriteMsg(msg p2p.Msg) error {
	if stream.UseDeadline {
...
func (server *LibP2PWhisperServer) connectToPeer(p *LibP2PPeer) error {
	...
	// Save the stream
	lps := newLibp2pStream(server, s).(*Stream)
...
func (server *LibP2PWhisperServer) Start() error {
	server.Host.SetStreamHandler(WhisperProtocolString, func(stream inet.Stream) {
		...
		lps := newLibp2pStream(server, stream).(*Stream)
